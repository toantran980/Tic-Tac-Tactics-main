...
class NPC():
    def __init__(self, x, y, width, height, screen):
        ...
        self.npc_images = [
            pygame.image.load(f'Character_Sprite/npc1.png'), 
            pygame.image.load(f'Character_Sprite/npc2.png'), 
            pygame.image.load(f'Character_Sprite/npc3.png'),
            pygame.image.load(f'Character_Sprite/npc4.png'),
            pygame.image.load(f'Character_Sprite/npc5.png'),
        ]

        self.resized_images = [pygame.transform.scale(image, (70, 70)) for image in self.npc_images]
        self.rects = [image.get_rect(topleft = (self.x, self.y)) for image in self.npc_images]
    ...
    def draw(self, camera_offset):
        ...
        for i in range(len(self.npc_images)):
            # Draw the image
            self.screen.blit(self.resized_images[i], (adjusted_x, adjusted_y))
            # Update the rectangle position to match the image position
            self.rects[i].topleft = (adjusted_x, adjusted_y) 
